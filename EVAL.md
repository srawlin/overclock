# overclock speed toolkit — eval, telemetry & experiments

Answers three questions on demand: **Is overclock fast? How do I measure it? How
do I A/B a speed change without losing agentic quality?**

## 1. Telemetry — do we have logs? **Yes.**

Location: `~/.overclock/agent/logs/metrics-*.jsonl` (or `$PI_CODING_AGENT_DIR/logs`).
...
`test/eval/run.mjs` runs a fixed task suite end-to-end through the **real overclock
...
OVERCLOCK_FAST=1 node test/eval/run.mjs # speed-optimized preset, A/B vs baseline
...
All levers are env-knobbed in `extensions/overclock/knobs.ts`. `OVERCLOCK_FAST=1`
applies a recommended, still-safe preset; any individual `OVERCLOCK_*` value overrides
...
|------|-----|----------------|--------------|------------------|
| keep-recent | `OVERCLOCK_KEEP_RECENT_TOOL_TOKENS` | 16000 / 8000 | Cap on how much *old* tool output stays verbatim. Smaller ⇒ smaller prompt ⇒ more cache reuse ⇒ lower TTFT on long sessions. | None — old results stub to a pointer, re-readable via tool. |
| hard budget | `OVERCLOCK_HARD_BUDGET_TOKENS` | 80000 / 20000 | Above this, drop to the tight keep window. | None. |
| tighten-keep | `OVERCLOCK_TIGHT_KEEP_TOKENS` | 4000 / 2000 | The tight-mode keep window. | None. |
| output reservation | `OVERCLOCK_MAX_COMPLETION_TOKENS` | 16384 / 8192 | Cerebras bills/limits on input + `max_completion_tokens`; a smaller cap slashes reserved quota + pacing sleep. | None — typical agentic turns emit <4k; long writes still covered. |
| explore model | `OVERCLOCK_EXPLORE_MODEL` | (same) | Run the read-only `explore` sub-agent on a cheaper/faster model. | Low — bounded + result-capped; raise only if explore quality drops here. |
| fast preset | `OVERCLOCK_FAST=1` | off | Combines the above into a conservative speed preset. | Low — use for A/B; revert a single knob if one task regresses. |
...
already, ~33–46k in), so `OVERCLOCK_FAST` is ~a wash here; it earns its keep on
...
2. **Tighten context** — `OVERCLOCK_FAST=1 node test/eval/run.mjs --repeat 3`. Expect a
   win on long sessions; confirm no task drops.
3. **Cheaper explore** — `OVERCLOCK_EXPLORE_MODEL=cerebras/gpt-oss-120b …` to keep the
   sub-agent path fast on a cheaper model (watches explore quality in the eval).
4. **Compare models** — `OVERCLOCK_MODEL=cerebras/qwen-3.8-27b` vs `gpt-oss-120b`.
